...
class approfile:
    ...
    sqlcred = ('{SQL Server}', 'Server', 'Database', 'UserId', 'Password') # SQl Credentials
    sqlcolname = 'HeaderName'
    #Query will be used in the table//////////////////////////////////////////////////////////////////////////
    RepName = 'AP Profile'
    itemqry = str("Select QueryHeader from Database.Schema.Table Where ReportName = '" + RepName + "'")
    ...
    for f in range(fcount):
        ...
        query = 'Select HeaderName from Database.Schema.Table where ID = 1'
        ...
        for ch in range(int(dfcnt)):
            try:
                qquery = str(f"Select QueryHeader from Database.Schema.Table Where HeaderName = '{str(df.columns[int(ch)].strip())}'")
