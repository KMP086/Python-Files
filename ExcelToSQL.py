...
class approfile:
    ...
    sqlcred = ('{SQL Server}', 'i21341', 'SQL_MNLDB', 'MNLPADeveloper$', 'cjw6fUQ8') # SQl Credentials
    sqlcolname = 'HeaderName'
    #Query will be used in the table//////////////////////////////////////////////////////////////////////////
    RepName = 'AP Profile'
    itemqry = str("Select QueryHeader from SQL_MNLDB.dbo.tblAPHeaderRef Where ReportName = '" + RepName + "'")
    ...
    for f in range(fcount):
        ...
        query = 'Select HeaderName from SQL_MNLDB.dbo.tblAPHeaderRef where ID = 1'
        ...
        for ch in range(int(dfcnt)):
            try:
                qquery = str(f"Select QueryHeader from SQL_MNLDB.dbo.tblAPHeaderRef Where HeaderName = '{str(df.columns[int(ch)].strip())}'")
